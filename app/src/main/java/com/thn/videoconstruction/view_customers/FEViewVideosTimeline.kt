...
import com.thn.videoconstruction.models.BitmapWithIndexDatas
import com.thn.videoconstruction.models.Recorded
...
class FEViewVideosTimeline : View {

    private val mImagePreviewPathList = ArrayList<String>()
    private var mBitmapList = ArrayList<BitmapWithIndexDatas>()
    private var mDx = 0f
    ...
    private val mRecordedList = ArrayList<Recorded>()
    ...
    var onStropSuccess: ((Recorded) -> Unit)? = null
        ...
        Observable.fromCallable<String> {
            if (imagePathList.size >= 10) {
                ...
                for (index in 0 until 10) {
                    ...
                    mBitmapList.add(BitmapWithIndexDatas(index, bitmap))
                    Loggers.e("path = ${imagePathList[index]}")
                }
            } else {
    ...
    fun loadImageVideo(videoPathList: ArrayList<String>) {
        Observable.fromCallable<String> {
            ...
            for (index in 0 until 10) {
                ...
                Loggers.e("path = ${videoPathList[index]}")
            }
    ...
    private fun drawResizedBitmap(imagePath: String): Bitmap {
        ...
        outBitmapSize = size
        Loggers.e("out size = $outBitmapSize")
    ...
    fun stopRecording(outPath: String) {
        ...
        mRecordedList.add(recorded)
        mStartRecordingOffset = 0f
        mEndRecordingOffset = 0f
        onStropSuccess?.invoke(recorded)
        invalidate()
    }
    ...
    fun setDataList(dataList: ArrayList<RecordedModels>) {
        mRecordedList.clear()
        for (item in dataList) {
            mRecordedList.add(Recorded(item.path, item.startOffset, item.endOffset))
        }
        invalidate()
    }
