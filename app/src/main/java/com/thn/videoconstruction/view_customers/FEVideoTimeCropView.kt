...
class FEVideoTimeCropView : View {
    ...
    fun loadImage(videoPath: String, width: Int) {
        ...
        Observable.fromCallable<String> {
            ...
            canvas.apply {
                while (dx < width) {
                    Loggers.e("time = ${(mMaxValue * dx / width)}")
    ...
    fun loadVideoImagePreview(videoPathList: ArrayList<String>, width: Int) {
        ...
        Observable.fromCallable<String> {
            ...
            var dx = 0f
            val numberImage = (width / (Utils.density(FEMainApp.getContext()) * 56)).toInt()
            Loggers.e("number image = $numberImage")
                ...
                for (videoPath in videoPathList) {
                    ...
                    if (currentTotal + videoDuration > targetTimeMs) {
                        ...
                        canvas.apply {
                            ...
                            dx += (Utils.density(FEMainApp.getContext()) * 56)
                        }
                        break
                    } else {
