...
class SlideShow(private val mImagePathList: ArrayList<String>):Serializable {
    ...
    private var mCurrentSlide: SlideRender
    private var mNextSlide: SlideRender
    private var mBackupSlide: SlideRender
    ...
    init {
        ...
        mCurrentSlide = SlideRender(mSlides[0], getBitmapFromHashMapV2(mSlides[0].imagePath))
        mNextSlide = SlideRender(mSlides[1], getBitmapFromHashMapV2(mSlides[1].imagePath))
        mCurrentSlideIndex = 2
        if (mSlides.size > 2) {
            mBackupSlide = SlideRender(mSlides[2], getBitmapFromHashMapV2(mSlides[2].imagePath))
        } else {
    ...
    fun getFrameByVideoTime(timeMiniSec:Int): FrameDatassss? {
        ...
        return FrameDatassss(mCurrentSlide.imageBitmap, mNextSlide.imageBitmap, progress, mCurrentSlide.slide.slideId, zoom)
    }

    fun getFrameByVideoTimeForRender(timeMiniSec:Int): FrameDatassss? {
        ...
        mCurrentSlide = SlideRender(mSlides[targetSlideIndex], getBitmapFromHashMapV2(mSlides[targetSlideIndex].imagePath))
        if(targetSlideIndex < (mSlides.size - 1)) {
            mNextSlide = SlideRender(mSlides[targetSlideIndex+1], getBitmapFromHashMapV2(mSlides[targetSlideIndex+1].imagePath))
        } else {
    ...
    fun getFrameDataByNumberFrame(numberFrame: Int): FrameDatassss? {
    ...
    private fun updateBackupSlide() {
        Thread{
            if(mNextSlide.slide.imagePath == "none") {
                mBackupSlide = SlideRender(mSlides[0], getBitmapFromHashMapV2(mSlides[0].imagePath))
            } else {
                for(index in 0..mSlides.size) {
                    ...
                    if(item.slideId == mNextSlide.slide.slideId) {
                        if(index == mSlides.size-1) {
                            ...
                            mBackupSlide = SlideRender(mSlides[index+1], getBitmapFromHashMapV2(mSlides[index+1].imagePath))
                        }
                        break
                    }
                }
    ...
    fun seekTo(timeMilSec:Int, onComplete:()->Unit) {
        ...
        Observable.fromCallable<String> {
            ...
            Loggers.e("target slide = $targetSlideIndex")
            mCurrentSlide = SlideRender(mSlides[targetSlideIndex], getBitmapFromHashMapV2(mSlides[targetSlideIndex].imagePath))
            if(targetSlideIndex < (mSlides.size - 1)) {
                mNextSlide = SlideRender(mSlides[targetSlideIndex+1], getBitmapFromHashMapV2(mSlides[targetSlideIndex+1].imagePath))
            } else {
    ...
    private fun getBlackSlide() : SlideRender {
        return SlideRender(Slide(View.generateViewId(), "none"), mBlackBitmap)
    }
