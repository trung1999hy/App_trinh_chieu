...
import com.thn.videoconstruction.BuildConfig
...
abstract class FEBaseActivity : AppCompatActivity() {
    ...
    override fun onStart() {
        ...
        dataController.setOnListenerFirebase(object : DataController.OnListenerFirebase {
            override fun onCompleteGetUser(user: User?) {
                user?.let {
                    FEMainApp.instance.getPreference()?.setValueCoin(it.coin)
                    txtCoin.text = FEMainApp.instance.getPreference()?.getValueCoin().toString()
                } ?: kotlin.run {
                    setDataBaseGold()
                }
            }
            ...
            override fun onFailure() {
                Toast.makeText(this@FEBaseActivity, "Có lỗi kết nối đến server!", Toast.LENGTH_LONG)
                    .show()
            }
        })
        dataController.user
    ...
    private fun setDataBaseGold() {
        val dataController = DataController(FEMainApp().deviceId)
        dataController.writeNewUser(FEMainApp().deviceId, 100)
        txtCoin.text = FEMainApp.instance.getPreference()?.getValueCoin().toString()
    ...
    protected fun showExportDialog(isEditVideo: Boolean = false, callback: (Int, Int) -> Unit) {
        if (!mExportDialogShowing) {
            ...
            view.saveButton.setOnClickListener {
                ...
                if (outQuality == 1080) {
                    FEMainApp.instance.getPreference()?.run {
                        if (this.getValueCoin() >= 2) {
                            AlertDialog.Builder(this@FEBaseActivity).setTitle("Notification")
                                .setMessage("Are you sure you want to export 1080p  minus 2 gold?") // Specifying a listener allows you to take an action before dismissing the dialog.
                                // The dialog is automatically dismissed when a dialog button is clicked.
                                .setPositiveButton(android.R.string.yes,
                                    DialogInterface.OnClickListener { dialog, which ->
                                            this.setValueCoin(getValueCoin() - 2)
                            ...
                            AlertDialog.Builder(this@FEBaseActivity).setTitle("Notification")
                                .setMessage("Open shop by coin ") //
                                .setPositiveButton(android.R.string.yes,
                                    DialogInterface.OnClickListener { dialog, which ->
                                        val intent = Intent(
                                            this@FEBaseActivity,
                                            FEPurchaseInAppActivity::class.java
                                        )
                                        startActivity(intent)
                                    }).setNegativeButton(android.R.string.no, null)
                                .setIcon(android.R.drawable.ic_dialog_alert).show()
    ...
    private fun updateGold() {
        val dataController = DataController(FEMainApp().deviceId)
        dataController.updateDocument(FEMainApp.instance.getPreference()?.getValueCoin() ?: 0)
    }
    ...
    fun showDownloadThemeDialog(
        linkData: ThemeLink, onClickDone: () -> Unit, onDownloadComplete: () -> Unit
    ) {
        ...
        val view = if (downloadViewHashMap[linkData.link] != null) {
            ...
            LayoutInflater.from(this)
                .inflate(R.layout.layout_download_theme_dialog, null, false)
        }
        ...
        if (downloadViewHashMap[linkData.link] == null) {
            ...
            Glide.with(this).load(Uri.parse(uriString)).into(view.themeIconInDownloadDialog)
    ...
    private fun onDownloadTheme(
        ...
        PRDownloader.download(link, Utils.themeFolderPath, "${fileName}.mp4").build()
            .setOnProgressListener {
                Loggers.e("progess = $it")
                val progress = it.currentBytes.toFloat() / it.totalBytes
                Loggers.e("progress = ${(progress * 100f).roundToInt()}")
                if (mDownloadDialogIsShow) runOnUiThread {
                ...
                override fun onError(error: Error?) {
                    Loggers.e("download error --> ${error?.connectionException?.message}")
                    runOnUiThread {
